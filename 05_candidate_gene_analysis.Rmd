...
ESR2<- ESR2[!grepl("PFT3", ESR2$sampleID),]
...
PRL<- PRL[!grepl("PFT3", PRL$sampleID),]
...
#VIPR2 X strength
VIPR2<- VIPR2[!grepl("PFT3", VIPR2$sampleID),]
...
# AVP strength
AVP<- AVP[!grepl("PFT3", AVP$sampleID),]
...
AVPR1B<- AVPR1B[!grepl("PFT3", AVPR1B$sampleID),]
...
OXT<- OXT[!grepl("PFT3", OXT$sampleID),]
...
figure<- ggarrange(plotlist=pit_plots, ncol=5, nrow=2, common.legend = TRUE)
...
ggsave(filename="../Candidate_gene_results/PIT_significant_PFT3rmStrength.png", figure, device="png", width=50, height=20, units="cm", dpi=300, bg="white")
```
...
VIPR2 was associated with both mean T and strength in the PIT. conduct AIC analysis to choose among models. 
...
lfc<- signif(results_int$log2FoldChange[results_int$gene=="VIPR2" & results_int$Tissue=="PIT"],2)
praw<- signif(results_int$pvalue[results_int$gene=="VIPR2" & results_int$Tissue=="PIT"],2)
padj=signif(results_int$padj[results_int$gene=="VIPR2" & results_int$Tissue=="PIT"],2)


a<- ggplot(VIPR2, aes(x=mean_T, y=VIPR2, color=Status)) + geom_point(size=4,pch=19) + labs(title="VIPR2 X Status * mean T",subtitle=substitute(paste("LFC=",a," p=",b," padj=", c),list(a=lfc, b=praw, c=padj)), x="mean Testosterone", y="Normalized Counts") + peri_theme + scale_colour_manual(values=status_cols2) + geom_smooth(method="lm", alpha=0.15)
a

```

```{r}
VIPR2_pit_lm_Status<- lm(VIPR2 ~ Batch  + Status, data=VIPR2)
VIPR2_pit_lm_meanT<- lm(VIPR2 ~Batch +  mean_T, data=VIPR2)
VIPR2_pit_lm_Str<- lm(VIPR2 ~Batch +  strength.all_study, data=VIPR2)
VIPR2_pit_lm_int<- lm(VIPR2 ~ Batch + Status*mean_T, data=VIPR2)

#against the NULL model
VIPR2_pit_lm_null<- lm(VIPR2 ~ Batch  , data=VIPR2)

model=c("VIPR2 ~ Batch", "VIPR2 ~ Batch + Status", "VIPR2 ~ Batch + mean T", "VIPR2 ~ Batch + Strength","VIPR2 ~ Batch + Status * mean T") 
AIC=c(AIC(VIPR2_pit_lm_null), AIC(VIPR2_pit_lm_Status),AIC(VIPR2_pit_lm_meanT),AIC(VIPR2_pit_lm_Str), AIC(VIPR2_pit_lm_int))
deltaAIC<- c(NA,AIC[1]-AIC[2], AIC[1]-AIC[3], AIC[1]-AIC[4], AIC[1]-AIC[5])

lm_p<- c(anova(VIPR2_pit_lm_null)$'Pr(>F)'[1],anova(VIPR2_pit_lm_Status)$'Pr(>F)'[2],anova(VIPR2_pit_lm_meanT)$'Pr(>F)'[2],anova(VIPR2_pit_lm_Str)$'Pr(>F)'[2],anova(VIPR2_pit_lm_int)$'Pr(>F)'[4])
DESeq2_raw_p=c(NA,
               results_pit$pvalue[results_pit$gene=="VIPR2" & results_pit$Test=="Status"],
               results_pit$pvalue[results_pit$gene=="VIPR2" & results_pit$Test=="mean T"],
               results_pit$pvalue[results_pit$gene=="VIPR2" & results_pit$Test=="Strength"],
               results_int$pvalue[results_int$gene=="VIPR2" & results_int$Tissue=="PIT"])



knitr::kable(data.frame(model, AIC,deltaAIC,lm_p, DESeq2_raw_p),row.names=FALSE, digits=3, padding=20,caption="Model validation in the pituitary (pit) for how VIPR2 expression is related to our interest variables") %>% kable_styling()
...
a<- ggplot(AR, aes(x=mean_T, y=AR, color=Status)) + geom_point(size=4,pch=19) + labs(title="AR X Status * mean T",subtitle=substitute(paste("LFC=",a," p=",b," padj=", c),list(a=lfc, b=praw, c=padj)), x="mean Testosterone", y="Normalized Counts") + peri_theme + scale_colour_manual(values=status_cols2) + geom_smooth(method="lm", alpha=0.15, formula=y ~ color*x)
a


p<- ggplot(AR, aes(x=mean_T, y=AR, color=Status)) + geom_point(size=0.6,pch=19) + labs(title="AR",subtitle=substitute(paste("LFC=",a," p=",b," q=", c),list(a=lfc, b=praw, c=padj)), x="mean testosterone", y="normalized counts") + peri_figure + scale_colour_manual(values=status_cols2) + geom_smooth(method="lm", alpha=0.15, size=0.4, se=FALSE) + theme(legend.position = "none", aspect.ratio=1)
p<- ggplot(AR, aes(x=mean_T, y=AR, color=Status)) + geom_point(size=0.6,pch=19) + peri_figure + scale_colour_manual(values=status_cols2) + labs(title="AR", subtitle="blah",x="mean T", y="normalized counts") + geom_smooth(method="lm", alpha=0.15, size=0.4, se=FALSE) + theme(legend.position = "none", aspect.ratio=1)
ggsave("../Candidate_gene_results/figure_PVN_AR_noerror.pdf", plot=p, device="pdf", height=1.60, width=1.60, units="in")
