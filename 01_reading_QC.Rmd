...
author: "Last Substantive Change October 2020"
output:
  html_document:
    toc: true
    toc_float: true
...
knitr::kable(table(files_key$Batch, files_key$Harvest_ID), caption="Number of individual libraries across sequencing Batchs", padding=20,)  %>% kable_styling()
knitr::kable(table(files_key$Batch, files_key$Tissue), caption="Number of libraries in each tissue across sequencing Batchs", padding=20,) %>% kable_styling()
knitr::kable(table(files_key$Batch, files_key$Status), caption="Number of libraries for each social status across sequencing Batchs", padding=20) %>% kable_styling()
```
...
Briefly, I used STAR to map the reads to the *Pipra filicauda* Refseq Genome V1 GCF_003945595.1. See the mapping script on the github [repository](https://github.com/periperipatus/PIFI_brain_transcriptome/tree/analysis_code)

The figure below shows the results of the summary statistics for each library

```{r read data, fig.width=7, fig.height=22}
...
mapping + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5, size = rel(0.5)), legend.position="top") + scale_y_continuous(expand=c(0,0)) + coord_flip()

#ggsave(filename="../data_filtered/figs/figure_mapping_summary_featureCounts.png", device="png", width=15, height=60, units="cm", dpi="print", bg="white")
...
The above plots identify 3 samples that mapped poorly, and these samples also had low RNA concentrations. Samples `r poor` will be excluded from further analyses. Then genes with an average count of greater than or equal to 5 will be excluded from downstream analysis. 
...
```{r Tissue read variation, fig.width=15,fig.height=10}
...
ggsave(filename="../data_filtered/figs/figure_PCA_all_tissues.png", device="png", width=30, height=30, units="cm", dpi="print")
```
...
```{r wgcna qc, fig.width=5, fig.height=25}
...
sampleTree<- as.dendrogram(sampleTree)
plot(sampleTree, main = "Sample clustering to detect outliers", sub="", xlab="height", cex.lab = 1.5, cex.axis = 1.5, cex.main = 2, horiz=TRUE)
...
```{r Batch effects figs, fig.width=8, fig.height=4}
...
```{r Year Effects figs, fig.width=8, fig.height=4}
par(mfrow=c(1,2))
...
run2run1_genes<- subset(run2run1, padj<0.05)
...
gonr2r1_genes<- rownames(res[which(res$padj<0.05),])
```
...
```{r random variable, echo=TRUE, fig.height=4, fig.width=8}
...
```{r replicate samples, fig.height=4, fig.width=8}
...
Now let's use ordination (1) and sample distance (2) to see how different they are from eachother. 
...
We appear to have broad batch effects  (with respect to Flow Cell (Batch) and with respect to Year sampled), but also there is not really a strong replicate effect as shown.
...
```{r wgnca dendrograms per tissue, include=FALSE}
...
png(file = "../data_filtered/figs/figure_SampleClustering.png", width = 2400, height = 2000, units="px", pointsize=16);
par(mfrow=c(3,4))
par(mar = c(0, 4, 2, 0))
for (set in 1:nSets)
...
```{r brain, fig.height=6, fig.width=7}
...
ggsave(filename="../data_filtered/figs/figure_PCA_brain_qcRepsRemoved_outlier_labs.png", device="png", width=30, height=25, units="cm", dpi="print")
```
...
Still seeing Year and Batch effects in the whole brain dataset.


## Pituitary

```{r pituitary, fig.height=6, fig.width=7}
