...
save(adjacency, TOM, file="../WGCNA_results/within_tissues/GCT_network.RData")
```
...
save(adjacency, TOM, file="../WGCNA_results/within_tissues/BSTm_network.RData")
```
...
fig<- ggarrange(a,b,c,d, nrow=2, ncol=2, labels=LETTERS[1:4])
fig

ggplot(wgcna_sum)
library(reshape2)
wider<-  melt(wgcna_sum, id.vars="Tissue",value.name="Shared")
wider<- wider[which(wider$variable!="total_modules" & wider$variable!="Individual"),]
wider<- droplevels(wider)
wider$Tissue<- factor(wider$Tissue, levels=c("GON","PIT","VMH","AH","PVN","POM","ICO","GCT","AI","TNA","LS", "BSTm"))
ggplot(wider, aes(x=Tissue, y=Shared, fill=variable)) + geom_bar(stat="identity", position=dodge) + scale_y_continuous(breaks=c(2,4,6,8,10), expand=c(0,0)) + peri_theme + labs(x="",y="Number Modules")
#x
```
