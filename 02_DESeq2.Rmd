...
author: "Last Substantive change January 2022"
output:
  html_document:
    toc: true
    toc_float: true
...
gh<- plotCounts(dd, gene="GH1", intgroup=c("Class","Status", "Tissue","time_kill","Plumage_Age","strength.all_study"), returnData=TRUE)
...
PC5 and PC10 seem to be associated with our interest variables. 

```{r wholebrain pc5 and 10, fig.height=18, fig.width=12}
library(nlme)
...
#summary(lm(PC5 ~ Year + Tissue*Status,data=test ))
m1<- lme(PC5 ~ Year + Tissue*Status, random=~1|Harvest_ID, data=test)
pc5test<- anova(m1)
...
```{r loadings, fig.height=10, fig.width=20}
...
The PCA plot shows that this makes the differences between batches worse, even though the eigencorplot (correlation between PC axis and variable) doesn't show it. 

In previous whole brain analyses I have attempted to run full brain models in DESeq2 that accounts for individual. However, these analyses do not run because of unequal sample sizes among tissues (DESeq2 issue). Further, the PCA results suggests that accounting for batch effects is not helpful. 
...
I will use the first two tissues to demonstrate the processes applied across all tissues, but code echoing will be suppressed in future tissues.
...
## check to see how our interest and nuisance variables are correlated to the PC axes. This will help decide on model form for DESeq.
eigencorplot(p, metavars = c("time_kill","Batch","Year", "Status"))
...
Conduct Gene Set enrichment analysis (GSEA) when there are few significant differentially expressed genes. This will leverage the whole list, but when we have a larger number I will use simple foreground/background enrichment.


```{r GON Status BP, echo=TRUE, fig.height=10, fig.width=18}
res_go<- Status_res
...
dd<- DESeqDataSetFromMatrix(countData=gon_data, colData=gon_behav, design= ~ Year + Status + mean_T)
dd<- DESeq(dd)
...
hplot<- ggplot(out, aes(x=mean_T, y=count, color=Class)) + geom_point(size=4) + labs(title=paste0(row.names(res[hub,])," LFC=",signif(mean_T_res$log2FoldChange[mean_T_res$gene==hub],2) ," padj=", signif(mean_T_res$padj[mean_T_res$gene==hub],2), " R^2=", signif(Rsq,2)),x="", y="Normalised Counts") + peri_theme + scale_colour_manual(values=status_cols, name="Age x Status")+ theme(legend.position="none")
```
...
res<- results(dd, alpha=0.1, name = "mean_T")
res<- rm_continuous_outliers(dd,res)
res<- res[order(res$padj),]
#summary(res)

res<- results(dd, alpha=0.1, name = "Status_territorial_vs_floater")
res<- rm_continuous_outliers(dd,res)
res<- res[order(res$padj),]
#summary(res)
...
write.csv(interaction_res, file="../DE_results/interactions_GON_mean_T_Status.csv", row.names=FALSE)
...
```{r GON strength BP, fig.height=10, fig.width=18}
res_go<- strength_res
res_go$logP<- -log(res_go$pvalue, 10)
res_go$logP<- ifelse(res_go$log2FoldChange<0, -(res_go$logP), res_go$logP)
res_go$gene<- rownames(res_go)
...
```{r PIT data set up}
keep<- c("data","key","behav","key_behav", "dodge", "peri_theme", "keep", "tissues_status", "tissues_mean_T", "tissues_final_T", "tissues_strength", "dj_col", "fromList", "get_intersect_members", "status_cols", "selFun","rm_continuous_outliers", "status_cols", "interactions","go_key","go2gene_bp", "go2gene_mf","go2name_bp","go2name_mf", "peri_figure")
...
```{r PIT Status GO, fig.height=10, fig.width=18}
res_go<- status_res
res_go$logP<- -log(res_go$pvalue, 10)
res_go$logP<- ifelse(res_go$log2FoldChange<0, -(res_go$logP), res_go$logP)
res_go$gene<- rownames(res_go)
...
write.csv(interaction_res, file="../DE_results/interactions_PIT_mean_T_Status.csv", row.names=FALSE)
...
write.csv(strength_res, file="../DE_results/results_PIT_Strength_withPFT3.csv", row.names=FALSE)
...
```{r VMH status GO,fig.height=10, fig.width=18}
res_go<- status_res
res_go$logP<- -log(res_go$pvalue, 10)
res_go$logP<- ifelse(res_go$log2FoldChange<0, -(res_go$logP), res_go$logP)
res_go$gene<- rownames(res_go)
...
write.csv(interaction_res, file="../DE_results/interactions_VMH_mean_T_Status.csv", row.names=FALSE)
...
write.csv(interaction_res, file="../DE_results/interactions_AH_mean_T_Status.csv", row.names=FALSE)
...
write.csv(interaction_res, file="../DE_results/interactions_PVN_mean_T_Status.csv", row.names=FALSE)
...
write.csv(interaction_res, file="../DE_results/interactions_POM_mean_T_Status.csv", row.names=FALSE)
...
write.csv(interaction_res, file="../DE_results/interactions_ICO_mean_T_Status.csv", row.names=FALSE)
...
write.csv(interaction_res, file="../DE_results/interactions_TNA_mean_T_Status.csv", row.names=FALSE)
...
write.csv(interaction_res, file="../DE_results/interactions_AI_mean_T_Status.csv", row.names=FALSE)
...
##treemapPlot(reducedTerms)
#
##scatterPlot(simMatrix, reducedTerms) + theme_bw() +  labs(title="Semantic similarity of GO terms") + #theme(plot.title = element_text(hjust = 0.5, size=12))
...
write.csv(interaction_res, file="../DE_results/interactions_LS_mean_T_Status.csv", row.names=FALSE)
...
gs<- GSEA(geneList, TERM2GENE = go2gene_bp, TERM2NAME = go2name_bp)
ridgeplot(gs)  + scale_fill_viridis() + peri_theme
```

```{r}
y<- enricher(interaction_res$gene[interaction_res$padj<0.1], universe=res_go$gene, TERM2GENE = go2gene_bp, TERM2NAME = go2name_bp)
...
p
ggsave("../DE_results/figure_LS_GO_str.pdf", plot=p, device="pdf", height=1.60, width=4.60, units="in")

#geom_bar(stat="identity", color="#3A2060", size=0.3, fill="#74638C") 
...
write.csv(interaction_res, file="../DE_results/interactions_BSTm_mean_T_Status.csv", row.names=FALSE)
...
go_mean_T<- y@result
write.csv(y@result, file="../DE_results/GSEA_BP_BSTm_interaction.csv", row.names=FALSE)
ego2<- as.data.frame(go_mean_T$ID)
colnames(ego2)<- "ID"
ego2$p.adjust<- go_mean_T$p.adjust
