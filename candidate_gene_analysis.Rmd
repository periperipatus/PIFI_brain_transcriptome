...
keep<- c("candidates","key_behav", "results_data","results_int","norm_data", "peri_theme","key","dodge", "status_cols", "dj_col", "colours","peri_presentation", "keep", "save_pheatmap_png", "data", "status_cols2")
...
Originally I was correcting for multiple comparisons using the raw p-values for the number of genes expressed in a tissue (using Benjamini-Hochberg). However, these were more conservative than the corrected p-values from DESeq2. Therefore, i only present the corrected p-values from DESeq2 based on the number of genes expressed in that tissue.



# LogfoldChange heatmaps

A simple way to present the candidate gene data is through this grid. These are the raw results that went into the publication - but some results will have been subsequently manually excluded on the basis of R^2 and strongly influential individuals. 

## Social Status

```{r Status,echo=TRUE, fig.width=12, fig.asp=0.8}
...
```{r mean T, fig.width=12, fig.asp=0.8}
...
```{r status by mean T, fig.width=12, fig.asp=0.8}
...
```{r strength, fig.width=12, fig.asp=0.8}
...
The next sections are scatter plots for each of the tissues showing what each of the genes are doing for manual validation. 
...
Here only the data with continuous variables were at all significant so these will be plotted with the extracted Normalized data. 

```{r GON candidate gene plots, fig.height=10, fig.width=10}
...
If not already identified as a possible interaction from multiple incidences in the single trait comparison, I also used DESeq2 to extract possible interactions. 
...
Note that the DESeq2 model for Status only is actually GeneExp ~ Batch + Status, and includes 4 additional samples. However, the linear models here don't differ in biological interpretation.
...
```{r pit candidate gene plots, fig.width=10, fig.height=35}
...
I am excluding AVP and OXT because the expression patterns are odd.
...
```{r VMH candidate gene plots, fig.width=10, fig.height=15}
...
Nothing convincingly significant after FDR correction!
...
No candidate genes significant in the AH
...
a<- ggplot(gct_norm, aes(x=mean_T, y=VIP, color=Status)) + geom_point(size=4,pch=19)  + labs(title="VIP X Status * mean T",subtitle=substitute(paste("LFC=",a," p=",b," padj=", c),list(a=lfc, b=praw, c=padj)), x="mean Testosterone", y="Normalized Counts") + peri_theme + scale_colour_manual(values=status_cols2) + geom_smooth(method="lm", alpha=0.15) + theme(legend.position="none")
a
```
...
c<- ggplot(gct_norm, aes(x=mean_T, y=PGR, color=Status)) + geom_point(size=4,pch=19)  + labs(title="PGR X Status * mean T",subtitle=substitute(paste("LFC=",a," p=",b," padj=", c),list(a=lfc, b=praw, c=padj)), x="mean Testosterone", y="Normalized Counts") + peri_theme + scale_colour_manual(values=status_cols2) + geom_smooth(method="lm", alpha=0.15) + theme(legend.position="none")
c
```
...
e<- ggplot(gct_norm, aes(x=mean_T, y=ESR1, color=Status)) + geom_point(size=4,pch=19) + labs(title="ESR1 X Status * mean T",subtitle=substitute(paste("LFC=",a," p=",b," padj=", c),list(a=lfc, b=praw, c=padj)), x="mean Testosterone", y="Normalized Counts") + peri_theme + scale_colour_manual(values=status_cols2) + geom_smooth(method="lm", alpha=0.15) + theme(legend.position="none")
e
```
...
data_vst<- read.csv("../../Data/analysis_code/data_filtering/data_VST_all_ReplicatesRemoved_antisense_V2.csv")
#data_vst<- data
