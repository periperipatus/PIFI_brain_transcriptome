...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning=FALSE, fig.align="center", comment=FALSE, fig.width=10, fig.height=7)
```


```{r libraries and presets, results='hide'}
library(WGCNA)
enableWGCNAThreads()
...
Like the DESeq2 document, the Gonads section will contain the code used throughout the the analysis to achieve the full set of results. First, each dataset is filtered conservatively as per the DESeq2 workflow, and then any batch effects identified in the DEseq workflow are removed here. Then we create a midweight bicorrelation weighted signed network using the recommended options. The Topological Overlap Matrix is also signed. 

<br>

## Data quality control

```{r GON data processing, echo=TRUE, results="hide"}
tissue<- "GON"
...
```{r GON soft-threshold, echo=TRUE, results="hide"}
...
The soft power selected will be the first power that that reaches scale free topology. 
...
```{r, GON making modules, echo=TRUE, results="hide"}
...
```{r}
plot(METree, main= "Clustering of module eigengenes in GON", xlab= "", sub= "")
...
```{r GON correlate with traits, echo=TRUE, results="hide"}
datTraits<- gon_key
...
wgcnasum<-  data.frame(Tissue=tissue, 
                                ...
                                Status=nrow(wgcnasum[wgcnasum$Status<0.05,]),
                                    mean_T=nrow(wgcnasum[wgcnasum$mean_T<0.05,]), strength=nrow(wgcnasum[wgcnasum$strength.all_study<0.05,]), total_modules=ncol(MEs))
...
```{r GON heatmap, echo=TRUE, fig.width=10, fig.height=7 }
...
### Pink module - mean testosterone

Here I extract the genes associated with a given module and explore them further. This module is potentially associated with mean testosterone. The WGCNA manual says that a good association will show a relationship between the module membership scores of each gene and the gene-significance for that gene.

```{r GON pink module, echo=TRUE}
whichModule="pink"
...
Next, I am going to explore the top 30 hub genes based on highest module membership
...
Now, let's plot the top gene in this module to see its relationship with mean testosterone
...
```{r GON pink network plotting TOM, echo=TRUE, results='hide'}
##### TOM BASED NETWORK ####
...
write.csv(ego@result, file=paste("../WGCNA_results/within_tissues/",tissue,"_", whichModule, "GO_BP.csv", sep=""))
...
The code for this tissue is mostly above, but each section the R environment is cleared to only keep the base data. 

```{r PIT data processing, echo=TRUE}
...
```{r PIT soft-threshold, results="hide"}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
The soft-power selected for `r tissue` is `r softPower`.
...
```{r, PIT making modules, results='hide'}
...
```{r PIT correlate with traits, fig.height=12, fig.width=7}
datTraits<- pit_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
```{r PIT extracting module information}
...
color<- merge(geneInfo0, datKME, by="gene") #these are from your original WGCNA analysis 
#head(color)
write.csv(as.data.frame(color), file = paste("../WGCNA_results/within_tissues/",tissue,"_results_ModuleMembership.csv", sep=""), row.names = FALSE)
MMPvalue<- merge(geneInfo0, MMPvalue, by="gene")
write.csv(MMPvalue, file=paste("../WGCNA_results/within_tissues/",tissue,"_results_ModuleMembership_P-value.csv", sep=""), row.names = FALSE)
...
```{r PIT floralwhite network plotting TOM}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
This module is too influenced by PFT3. This is a sample I removed in the DESeq2 analysis. Disregard. 

<br>

### pink4 module - strength
...
```{r PIT pink4 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
knitr::kable(ego@result[1:10,], caption=paste0("BP enriched GO terms in ", whichModule," module in ",tissue), row.names = FALSE) %>% kable_styling()
write.csv(ego@result, file=paste("../WGCNA_results/within_tissues/",tissue, whichModule, "_GO_BP.csv", sep=""))
...
```{r VMH soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r VMH making modules, results='hide'}
...
```{r VMH correlate with traits, fig.height=12, fig.width=7}
datTraits<- vmh_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
```{r VMH extracting module information}
...
### lightcoral module - Status/mean testosterone
...
The top hub gene was not significantly associated with either mean_T or Status in the DESeq2 analysis. 

```{r VMH lightcoral network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r AH soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r AH making modules, results='hide'}
...
```{r AH correlate with traits, fig.height=15, fig.width=7}
datTraits<- ah_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
```{r AH mediumpurple1 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
...
Also the inverse module of the mediumpurple1 module based on the intermodule correlation plot
...
```{r AH tan network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
...
```{r PVN soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r PVN making modules, results='hide'}
...
```{r PVN white network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
Note that the PFT2 POM sample was removed from this analysis as it was previously found to be an outlier.
...
```{r POM soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r POM making modules, results='hide'}
...
```{r POM correlate with traits, fig.height=20, fig.width=7}
datTraits<- pom_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
### floralwhite module - Status/mean T
...
```{r POM floral white network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r POM pink3 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
### mediumpurple1 - mean testosterone
...
```{r POM mediumpurple1 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r POM sienna3 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r ICO soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r ICO making modules, results='hide'}
...
```{r ICO correlate with traits, fig.height=15, fig.width=7}
datTraits<- ico_key
all.equal(rownames(datExpr0), rownames(datTraits))
...
write.csv(GS, file=paste("../WGCNA_results/within_tissues/",tissue, "results_GeneSignificance.csv", sep=""), row.names = FALSE)
...
```{r ICO lavenderblush2 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
## get names of genes from the module
probes <- names(datExpr0)
...
### darkturquoise - mean testosterone
...
```{r ICO darkturquoise network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r ICO maroon network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r GCT soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r GCT making modules, results='hide'}
...
```{r GCT correlate with traits, fig.height=12, fig.width=7}
datTraits<- gct_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
There appears to be a strong correlation between *skyblue2* and *green4*, which are associated with strength.
...
```{r GCT skyblue2 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
I'm not convinced. Let's move on to correlations with stronger p-values

<br>

### lightpink3 - mean testosterone
...
```{r GCT lightpink3 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r GCT antiquewhite1 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r GCT antiquewhite2 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r GCT coral1 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r TNA soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r TNA making modules, results="hide"}
...
```{r TNA correlate with traits, fig.height=12, fig.width=7}
datTraits<- tna_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
Based on inspection of the module memberships it seems that *coral1* is the inverse of *blue* and/or *lightyellow*
...
```{r TNA coral1 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r TNA blue network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r TNA lightyellow network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r AI soft-threshold, results="hide"}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r AI making modules, results='hide'}
...
```{r AI correlate with traits, fig.height=15, fig.width=7}
datTraits<- ai_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
```{r AI thistle1 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r AI skyblue2 network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
### palevioletred3 - mean testosterone
...
```{r AI palevioletred network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r LS soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r LS making modules, results='hide'}
...
```{r LS correlate with traits, fig.height=12, fig.width=7}
datTraits<- ls_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
gone<- plotCounts(dd, gene="YPEL2", intgroup=c("strength.all_study", "Status"), returnData=TRUE)
ggplot(gone, aes(x=strength.all_study, y=count)) + geom_point(size=2) + labs(title="jub-gene YPEL2",x="Social network strength", y="Normalised Counts") + peri_theme 
```

```{r LS darkmagenta network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r, fig.height=5, fig.width=5}
gone<- plotCounts(dd, gene="KPNA5", intgroup=c("strength.all_study", "Status"), returnData=TRUE)
ggplot(gone, aes(x=strength.all_study, y=count)) + geom_point(size=2) + labs(title="Hub-gene KPNA5",x="social network strength", y="Normalised Counts") + peri_theme 
```

```{r LS black network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r BSTm soft-threshold, results='hide'}
powers<- c(seq(1, 11, by = 1), seq(12, 26, by = 2))
...
```{r BSTm making modules, results='hide'}
...
```{r BSTm correlate with traits, fig.height=12, fig.width=7}
datTraits<- bstm_key
#all.equal(rownames(datExpr0), rownames(datTraits))
...
```{r BSTm darkred network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r BSTm darkmagenta network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
```{r BSTm saddlebrown network plotting TOM, results='hide'}
##### TOM BASED NETWORK ####
## get names of genes from the module
probes <- names(datExpr0)
inModule <- moduleColors==whichModule #creates logical vector of whether that gene is in the module
...
wgcna_sum<- do.call("rbind", wgcna_summary)
write.csv(wgcna_sum, file="../WGCNA_results/within_tissues/ALL_modules_summary.csv", row.names=FALSE, quote=FALSE)


a<- ggplot(wgcna_sum, aes(x=Tissue, y=Individual/total_modules)) + geom_bar(stat="identity") + labs(title="Individual", y="No sig. modules p<0.05", x="")
b<- ggplot(wgcna_sum, aes(x=Tissue, y=Status/total_modules)) + geom_bar(stat="identity") + labs(title="Status", y="No sig. modules p<0.05", x="")
c<- ggplot(wgcna_sum, aes(x=Tissue, y=mean_T/total_modules)) + geom_bar(stat="identity") + labs(title="mean testosterone", y="No sig. modules p<0.05", x="")
d<-ggplot(wgcna_sum, aes(x=Tissue, y=strength/total_modules)) + geom_bar(stat="identity") + labs(title="social network strength", y="No sig. modules p<0.05", x="")

fig<- ggarrange(a,b,c,d, nrow=2, labels=LETTERS[1:4])
fig
```
