...
keep<- c("candidates","tissue_order", "tissue_cols","tissue_colors", "topgenes", "go_terms","peri_theme","peri_figure", "go2gene_bp","go2gene_mf", "go_obo","go2name_bp", "go2name_mf","keep", "key_behav", "data", "dodge","status_cols")
```
...
pheatmap(pvalue_cor, color=color[42:100], annotation_row=exp, border_color = NA, display_numbers = signif(pvalue_cor,1), annotation_colors = ann_cols, clustering_distance_cols = "correlation", clustering_distance_rows = "correlation", clustering_method="complete")
...
ggsave("../DE_results/figure_braintop_status.pdf", plot=p, device="pdf", height=3, width=5.5, units="in")
```
...
setwd("C:/Users/perif/OneDrive - East Carolina University/Manakins/Pipra Brain/PIFI_brain_transcriptome/analysis")
```
```{r meanT volcano plotting, fig.height= 20, fig.width=15}
g<- ggarrange(plotlist=plots,ncol=3, nrow=4, common.legend = TRUE)
...
pheatmap(pvalue_cor, color=color[44:100], annotation_row=exp, border_color = NA, display_numbers = signif(pvalue_cor,1), annotation_colors = ann_cols, clustering_distance_cols = "euclidean", clustering_distance_rows = "euclidean", clustering_method="complete")
...
for(i in 1:5){
  ...
  plots[[i]]<-  ggplot(dat, aes(x=mean_T, y=count)) + geom_point(size=2, aes(color=Status)) + labs(title=paste0(name, " p(median)=", formatC(10^-abs(brain_go$p_combo[brain_go$gene==name]),2, format="e")),x="", y="Normalised Counts") + peri_figure + scale_colour_manual(values=status_cols[2:3]) + facet_wrap(. ~Tissue, ncol=5) + geom_text(data=result, aes(x=x, y=y, label=label))
}
...
setwd("C:/Users/perif/OneDrive - East Carolina University/Manakins/Pipra Brain/PIFI_brain_transcriptome/analysis")
```
```{r strength volcano plotting, fig.height= 20, fig.width=15}
g<- ggarrange(plotlist=plots,ncol=3, nrow=4, common.legend = TRUE)
...
ggsave(file="../Overlap_results/RRHO2/figure_RRHO_strength_scaled.pdf", g, units="in", width=20, height=20)


g<-grid.arrange(grobs= plots, layout_matrix = m)
ggsave(file="../Overlap_results/RRHO2/figure_RRHO_strength_unscaled.pdf", g, units="in", width=25, height=10)
```
...
for(i in 1:6){
  ...
  plots[[i]]<-  ggplot(dat, aes(x=strength.all_study, y=count)) + geom_point(size=2, position=dodge, aes(color=Status)) + labs(title=paste0(name, " p(median)=", formatC(10^-abs(brain_go$p_combo[brain_go$gene==name]),2, format="e")),x="", y="Normalised Counts") + peri_figure + scale_colour_manual(values=status_cols[2:3]) + facet_wrap(. ~Tissue, ncol=5) + geom_text(data=result, aes(x=x, y=y, label=label))
}
...
p<- ggplot(pos_neg_int, aes(x=tissue, y=res, fill=type)) + geom_bar(stat="identity") + peri_theme + scale_y_continuous(expand=c(0,0), limits=c(-1200, 1200)) + labs(title="Status x mean T", x="", y="") + scale_fill_manual(values=pos_cols) + geom_hline(yintercept=0)+ theme(legend.position = "none") + geom_text(aes(label=abs_val),vjust=pos_neg_int$res < 0)+ theme(plot.margin = unit(c(1,1,0,0), "lines"))
p
ggsave("../Overlap_results/figure_StatusmeanT_bars.pdf", plot=p, device="pdf", height=1.75, width=1.75, units="in")
...
#some code to test something for Chris
histog<- ggarrange(xy_status, xy_t, xy_str, nrow=3)
ggsave("../Overlap_results/figure_corr_hist.pdf", plot=histog, device="pdf", height=1.75, width=1.75, units="in")

```


```{r number of DE genes, eval=FALSE}
...
ggsave("../Overlap_results/figure_corr_degs.png", plot=corrs, device="png", bg="white")
...
ggplot(gene_correlations, aes(x=tissue, y=R, fill=comparison)) + geom_bar(stat="identity",position=dodge) + scale_fill_manual
